	...
	testCases := []struct {
	...
	}{
		{
			...
			mockFunc: func(t *testing.T) httpClient {
				...
				cli.EXPECT().sendGetRequest(path.Join(_apiPathPrefix, "/user/me"), "", gomock.Any()).Return(errors.New("mock error"))
				return cli
			},
			expectedUser:  nil,
		...
		},
		{
			...
			mockFunc: func(t *testing.T) httpClient {
				ctrl := gomock.NewController(t)
				cli := NewMockhttpClient(ctrl)
				cli.EXPECT().sendGetRequest(path.Join(_apiPathPrefix, "/user/me"), "", gomock.Any()).Return(nil)
				return cli
			},
			expectedUser:  &User{},
			expectedError: "",
